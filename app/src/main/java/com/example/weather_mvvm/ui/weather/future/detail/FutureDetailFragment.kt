...
import com.example.weather_mvvm.internals.DateNotFoundException
...
class FutureDetailFragment : ScopedFragment(), KodeinAware {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        val safeArgs = arguments?.let { FutureDetailFragmentArgs.fromBundle(it) }
        val date = LocalDateConverter.stringToDate(safeArgs?.dateString) ?: throw DateNotFoundException()

        viewModel = ViewModelProviders.of(this, viewModelFactoryInstanceFactory(date)).get(FutureDetailViewModel::class.java)
    ...
    private fun bindUI() = launch(Dispatchers.Main) {
        ...
        futureWeather.observe(viewLifecycleOwner, Observer { weatherEntry ->
            ...
            Glide.with(this@FutureDetailFragment)
